import {
    ...
    seedCityOnDemand,
    backfillGeminiForCity,
} from "../services/seedCityService.js";


export async function backfillGeminiController(req, res) {
    try {
        const cityId = req.query.cityId;
        const limit = Number(req.query.limit ?? 200);
        const batch = Number(req.query.batch ?? 15);

        if (!cityId) {
            return res.status(400).json({ message: "cityId is required" });
        }

        const stats = await backfillGeminiForCity({
            cityId,
            limit,
            batchSize: batch,
        });
        ...
        console.error("backfillGeminiController error:", err);
